def HasDupes(word_to_dupe_check): # checks for dupes
    ...
    if len(letters) > len(no_rep): # returns true if there are more letters than unique letters
        return True
    else:
        return False
...
def main():
    ...
    print('------------------------------------------------------') # divides secret collection from guess collection
        ...
        if guess == secret_word:
            ...
            for i in secret_word_list:
                for j in guess_list:
                    if i == j:
                        score = score + 1
            print(color_start + guess + color_end + ': ' + color_start + str(score) + color_end + '\n')
